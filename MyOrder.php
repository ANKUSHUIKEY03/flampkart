...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
     <title>Online Shopping in Cities shop</title>
  ...
  <body style="background:#f1f3f6">
      ...
      $query = mysqli_query($con,"SELECT DISTINCT invoice_no FROM orders  INNER JOIN customer
      ...
      while($order_data = mysqli_fetch_array($query)): ?>
        ...
        <div class="col-lg-12">
          <div class="card mb-4">
                  ...
                  <?php
                    $order = mysqli_query($con,"SELECT * FROM orders INNER JOIN customer
                    ON orders.c_id=customer.c_id
                    INNER JOIN product
                      ON orders.p_id=product.p_id
                    INNER JOIN vendor
                                 ...
                                 if($data['order_status']=='Active' || $data['order_status']=='Delivered'):
                                       ...
                                       //echo date('Y-m-d h:i:s', time());
                                        if( date('Y-m-d h:i:s', time())>$del_time):
                                          $o_id=$data['o_id'];
                                          $delevered=mysqli_query($con, "UPDATE orders
                                            set
