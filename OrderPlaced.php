  ...
  <body>
      ...
      <?php
        if(isset($_SESSION['customer'])):
          ...
          $p_id=$_GET['p_id'];
          $c_query=mysqli_query($con,"SELECT * FROM customer WHERE c_email='$log'");
          $c_row=mysqli_fetch_array($c_query);

          $p_query=mysqli_query($con,"SELECT * FROM product INNER JOIN vendor on  vendor.v_email=product.v_email WHERE p_id='$p_id'");
          $p_row=mysqli_fetch_array($p_query);
          $o_query=mysqli_query($con,"SELECT * FROM orders WHERE p_id='$p_id'");
          $o_row=mysqli_fetch_array($o_query);
